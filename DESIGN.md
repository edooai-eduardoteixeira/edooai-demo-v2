...
- **Colors**: Both segments are brand-derived (equally important cost components). Bottom segment (referrer) = brand at 20% opacity (`rgba(102, 0, 31, 0.2)`); top segment (referee) = `var(--color-brand)` (#66001F, full brand). Keeps the chart visually connected to the dashboard while making both segments distinguishable
